...
import { IonButtons, IonButton, IonIcon, useIonAlert, useIonToast, IonContent, IonItem, IonLabel, IonPopover, IonImg } from '@ionic/react';
...
import { PostInfo, PostLike, User, Notification } from '../Types/entitinities';
...
import { UniversalFollowButton } from '../pages/Notifications';
...
export default function RenderDiscoverPagePosts(props: { post: PostInfo }) {
    ...
    useEffect(() => {
        setIsFollowingUser(post.user ? followings.filter(following => following.email === post.user!.email).length > 0 : false);
    }, [followings])
            ...
            <div className="post--topbar--container">
                <div className="username--container">
                    ...
                    <IonButton onClick={() => { user.email == post.user?.email ? history.push("/profile") : history.push("/UserProfile", { user: post.user }) }} className="username--image--tag" fill="clear">
                        <span className="username--image--tag--span">@{post.user?.username || post.username.split("@")[0]}</span>
                    </IonButton>
                    {verifiedMember.includes(`${post.user?.username}`) ? <img height={15} width={15} src={verified} /> : ''}

                </div>
                <span slot="end">
                    {post.user && <UniversalFollowButton className="button--notif--follow" appUser={post.user} />}
                </span>
                ...
                <div className="buttons--container">
                    ...
                    <div className="button--container--right">
                        <IonButtons slot="secondary">
                            <IonButton onClick={() => history.push('/likes', { post, likes })} className="likes--button right--buttons" fill='clear'>
                                {likeLength()}
                            </IonButton>
